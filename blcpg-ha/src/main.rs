...
async fn main() -> Result<()> {
    ...
    info!(
        "Starting BLC PostgreSQL HA Agent v{}",
    ...
    let pg_conn = match postgresql::connect(&config.postgresql).await {
        ...
        Err(e) => {
            warn!(
                "Failed to connect to PostgreSQL: {}. Agent will start in degraded mode.",
                e
            );
    ...
    let raft_node =
        Arc::new(raft::RaftNode::new(config.raft.clone(), cluster_manager.clone()).await?);
