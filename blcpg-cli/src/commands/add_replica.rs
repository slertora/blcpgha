...
pub async fn execute(
    ...
    if cluster_status.nodes.iter().any(|node| node.node_id == target) {
        if !force {
            println!("{}", "ERROR: Target node already exists in cluster!".red().bold());
            println!("Use --force to override this check.");
            return Ok(());
        } else {
            println!("{}", "WARNING: Target node already exists, proceeding with --force".yellow().bold());
        }
    }
    ...
    if target_has_postgresql && !force {
        println!("{}", "ERROR: Target node already has PostgreSQL installed!".red().bold());
        println!("Use --force to override this check.");
        return Ok(());
    }

    // Select best source for backup
    println!("{}", "Step 2: Selecting best source for backup...".bold().cyan());
    ...
    println!("Source Health Score: {:.2}%", get_node_health_score(&cluster_status.nodes, &selected_source));
    println!("Source Replication Lag: {}", get_replication_lag_text(&cluster_status.nodes, &selected_source));
    println!();

    // Validate source node (skip validation in demo mode)
    if !demo {
        let source_node = cluster_status.nodes
            .iter()
        ...
        if !source_node.is_healthy {
            println!("{}", "WARNING: Selected source node is not healthy!".red().bold());
        ...
        if let Some(lag) = source_node.replication_lag_seconds {
            if lag > 10.0 {
                println!("{}", "WARNING: High replication lag detected on source!".red().bold());
    ...
    println!("  PostgreSQL Installation: {}", if target_has_postgresql { "Already installed".yellow() } else { "Will install".green() });
    ...
    // Final confirmation
    let response = get_user_confirmation(&format!("Do you want to add {} as a replica using {} as source? (y/N): ", target, selected_source), demo)?;
    if !response {
        println!("Operation cancelled.");
        return Ok(());
    }
    ...
    match execute_add_replica(target, &selected_source, wait_sync, demo).await {
        Ok(_) => {
            ...
            if let Some(new_replica) = updated_cluster.nodes.iter().find(|n| n.node_id == target) {
                println!("New Replica Status: {}", if new_replica.is_healthy { "Healthy".green() } else { "Unhealthy".red() });
                println!("Health Score: {:.2}%", new_replica.health_score);
                println!("Replication Lag: {}", get_replication_lag_text(&updated_cluster.nodes, target));
            }
        }
        Err(e) => {
...
async fn check_postgresql_installed(node_id: &str, demo: bool) -> Result<bool> {
    ...
    if demo {
        println!("  [DEMO MODE] Simulating PostgreSQL check on {}...", node_id);
    ...
    println!("  [SIMULATED] Executing PostgreSQL checks on {}...", node_id);
...
async fn select_best_source(nodes: &[crate::client::NodeInfo], source: &str, demo: bool) -> Result<String> {
    if demo {
    ...
    match source {
        "auto" => {
            ...
            if let Some(primary) = nodes.iter().find(|node| node.is_primary && node.is_healthy) {
                ...
                let healthy_replicas: Vec<_> = nodes.iter()
                ...
                println!("  Auto-selected best replica: {}", best_candidate.node_id.bold());
...
async fn execute_add_replica(
    ...
    if demo {
        println!("    [DEMO MODE] Simulating pg_basebackup from {} to {}...", source, target);
...
async fn install_postgresql(node_id: &str) -> Result<()> {
    ...
    let _install_commands = vec![
        ...
        format!("ssh {} 'sudo apt-get install -y postgresql-15 postgresql-client-15'", node_id),
...
async fn create_backup(source: &str, target: &str) -> Result<()> {
    ...
    println!("    [SIMULATED] Creating backup from {} to {}...", source, target);
...
async fn wait_for_sync(node_id: &str) -> Result<()> {
    ...
    println!("    [SIMULATED] Waiting for replication to sync on {}...", node_id);
    std::thread::sleep(std::time::Duration::from_secs(2));
