...
pub async fn execute(client: &impl ApiClientTrait) -> Result<()> {
    ...
    println!("  Is Leader: {}", if status.raft.is_leader { "YES".green().bold() } else { "NO".yellow() });
    println!("  Leader ID: {}", status.raft.leader_id.as_ref().unwrap_or(&"None".to_string()));
    println!("  Commit Index: {}", status.raft.commit_index);
    println!("  Last Applied: {}", status.raft.last_applied);
    println!("  Peers: {}", if status.raft.peers.is_empty() { "None".yellow().to_string() } else { status.raft.peers.join(", ") });
    println!();

    // PostgreSQL Information
    println!("{}", "POSTGRESQL STATUS:".bold().cyan());
    println!("  Is Primary: {}", if status.health.is_primary { "YES".green().bold() } else { "NO".yellow() });
    ...
    println!("  Cluster Leader: {}", cluster_status.leader.as_ref().unwrap_or(&"None".to_string()));
    println!();
    ...
    for node in &cluster_status.nodes {
        ...
        println!("    Raft Leader: {}", if node.is_leader { "YES".green() } else { "NO".yellow() });
        println!("    PostgreSQL Primary: {}", if node.is_primary { "YES".green() } else { "NO".yellow() });
        println!("    Health: {}", if node.is_healthy { "Healthy".green() } else { "Unhealthy".red() });
        println!("    Health Score: {:.2}%", node.health_score);
        println!("    Replication Lag: {}", get_replication_lag_text(node.replication_lag_seconds));
    ...
    let raft_leader = if status.raft.is_leader { 
        ...
        status.raft.leader_id.as_ref().unwrap_or(&"None".to_string()).to_string() 
    };
    ...
    println!("  Overall Status: {}", if status.health.is_healthy { "Healthy".green() } else { "Unhealthy".red() });
