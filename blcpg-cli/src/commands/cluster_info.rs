...
pub async fn execute(client: &impl ApiClientTrait, detailed: bool) -> Result<()> {
    ...
    if detailed {
        ...
        println!("Leader: {}", cluster_status.leader.as_ref().unwrap_or(&"None".to_string()));
        println!();
        
        println!("{}", "Nodes:".bold());
        for node in &cluster_status.nodes {
            println!("  Node ID: {}", node.node_id.bold());
            println!("    Leader: {}", if node.is_leader { "Yes".green() } else { "No".yellow() });
            println!("    Health: {}", if node.is_healthy { "Healthy".green() } else { "Unhealthy".red() });
            println!("    Health Score: {:.2}%", node.health_score);
            println!("    Primary: {}", if node.is_primary { "Yes".green() } else { "No".yellow() });
        ...
        let table_data: Vec<ClusterNodeTable> = cluster_status.nodes
            .iter()
