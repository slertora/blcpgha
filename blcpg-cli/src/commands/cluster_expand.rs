...
pub async fn execute(
    ...
    if !demo {
        println!(
            "{}",
            "Step 1: Validating expansion requirements...".bold().cyan()
        );
        ...
        if cluster_status.healthy_nodes < 2 {
            println!(
                "{}",
                "WARNING: Low number of healthy nodes!".yellow().bold()
            );
        ...
        if let Some(primary) = primary_node {
            if !primary.is_healthy {
                println!(
                    "{}",
                    "WARNING: Primary node is not healthy!".yellow().bold()
                );
                let response =
    ...
    let response = get_user_confirmation(
        &format!(
            "Do you want to expand cluster with {} new node(s)? (y/N): ",
            count
        ),
        demo,
    )?;
...
pub(crate) fn generate_new_node_ids(
    existing_nodes: &[crate::client::NodeInfo],
    count: u32,
) -> Result<Vec<String>> {
