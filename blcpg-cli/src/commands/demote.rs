...
pub async fn execute(client: &impl ApiClientTrait, node_id: &str) -> Result<()> {
    ...
    let healthy_replicas: Vec<_> = cluster_status
        .nodes
        .iter()
    ...
    if !healthy_replicas.is_empty() {
        ...
        println!(
            "  New Primary will be: {}",
            best_candidate.node_id.green().bold()
        );
    } else {
        println!(
            "  New Primary: {}",
            "NONE (Cluster will be without primary)".red().bold()
        );
    }
    println!();

    // Final confirmation
    let response = get_user_confirmation(&format!(
        "Do you want to demote node '{}'? (y/N): ",
        target_node.node_id
    ))?;
    if !response {
        println!("Demotion cancelled.");
    ...
    match client.demote_node(&target_node.node_id).await {
        Ok(_) => {
            ...
            println!(
                "Primary Node: {}",
                if updated_status.health.is_primary {
                    "This node".green()
                } else {
                    "None".yellow()
                }
...
fn select_best_candidate<'a>(
    nodes: &'a [&'a crate::client::NodeInfo],
