...
pub async fn execute(
    ...
    if !demo {
        println!(
            "{}",
            "Step 1: Validating backup conditions...".bold().cyan()
        );
    ...
    println!(
        "  Compression Ratio: ~{:.1}%",
        (1.0 - compression as f64 / 10.0) * 100.0
    );
    if encrypt {
    ...
    println!(
        "  Database Impact: {}",
        "Minimal (consistent backup)".green()
    );
    println!("  Network Usage: {}", format_bytes(estimated_size));
    println!(
        "  Storage Required: {}",
        format_bytes(estimated_size * (compression as f64 / 10.0) as u64)
    );
    ...
    let response = get_user_confirmation(
        &format!(
            "Do you want to start backup of {}? (y/N): ",
            target_node.node_id
        ),
        demo,
    )?;
    ...
    .await
    {
        Ok(backup_info) => {
            ...
            println!(
                "Compression Ratio: {:.1}%",
                backup_info.compression_ratio * 100.0
            );
            println!(
                "Encryption: {}",
...
fn select_best_backup_source<'a>(
    ...
    if let Some(primary) = nodes.iter().find(|node| node.is_primary && node.is_healthy) {
        println!(
            "  Selected primary node: {} (optimal for backups)",
...
fn calculate_estimated_backup_size(
...
async fn execute_backup(
    ...
    if demo {
        println!(
            "  [DEMO MODE] Simulating backup of {}...",
            target_node.node_id
        );
