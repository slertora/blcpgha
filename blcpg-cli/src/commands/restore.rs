...
pub async fn execute(
    ...
    println!("  Verify Backup: {}", if verify { "Enabled".green() } else { "Disabled".yellow() });
    println!();

    // Validate backup ID format
    if !backup_id.starts_with("backup-") {
        println!("{}", "WARNING: Backup ID format seems invalid!".yellow().bold());
    ...
    if !demo {
        println!("{}", "Step 1: Validating restore conditions...".bold().cyan());
    ...
    let point_in_time_dt = if let Some(pit_str) = &point_in_time {
        match parse_point_in_time(pit_str) {
            Ok(dt) => {
                println!("  ✅ Point-in-time parsed successfully: {}", dt.format("%Y-%m-%d %H:%M:%S"));
                Some(dt)
            }
            Err(e) => {
    ...
    if let Some(pit) = point_in_time_dt {
        println!("  Point-in-Time: {}", pit.format("%Y-%m-%d %H:%M:%S").to_string().bold());
    }
    println!();
    ...
    println!("  Backup Verification: {}", if verify { "Required".green() } else { "Skipped".yellow() });
    println!();
    ...
    if target_node.is_primary {
        ...
        println!("  Database Impact: {}", "Minimal - Replica node only".green());
    ...
    // Final confirmation
    let response = get_user_confirmation(&format!("Do you want to start restore of backup {} to {}? (y/N): ", backup_id, target_node.node_id), demo)?;
    if !response {
        println!("Restore cancelled.");
        return Ok(());
    }
    ...
    // Execute the restore
    match execute_restore(client, backup_id, target_node, restore_type, point_in_time_dt, verify, demo).await {
        Ok(restore_info) => {
            ...
            if let Some(pit) = restore_info.point_in_time {
                println!("Point-in-Time: {}", pit.format("%Y-%m-%d %H:%M:%S").to_string());
            }
            println!("Size: {}", format_bytes(restore_info.size_bytes));
            println!("Status: {}", restore_info.status);
            println!("Verification: {}", if restore_info.verification_passed { "Passed".green() } else { "Failed".red() });
        }
        Err(e) => {
...
fn select_best_restore_target<'a>(nodes: &'a [crate::client::NodeInfo], demo: bool) -> Result<&'a crate::client::NodeInfo> {
    if demo {
...
async fn execute_restore(
    ...
    if demo {
        println!("  [DEMO MODE] Simulating restore of backup {} to {}...", backup_id, target_node.node_id);
